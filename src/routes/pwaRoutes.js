...
router.get('/manifest.webmanifest', asyncHandler(async (req, res) => {
  ...
  const name = settings.officialName || 'English Center';
  const shortName = settings.shortName || name;
  ...
  res.json({
    ...
    description: settings.description || `Plateforme éducative de ${name}`,
    start_url: '/',
    scope: '/',
    display: 'standalone',
    orientation: 'any',
    background_color: '#F5F7FB',
    theme_color: settings.primaryColor || '#1D4ED8',
