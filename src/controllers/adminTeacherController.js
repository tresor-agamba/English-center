...
function password(body) {
  return passwords.validatePassword(body.password, body.passwordConfirmation);
}
...
async function create(req, res) {
  try {
    const teacher = await service.create({ ...identity(req.body), passwordHash: await passwords.hashPassword(password(req.body)), mustChangePassword: true });
...
async function resetPassword(req, res) { const teacher = await get(req.params.id); await service.resetPassword(teacher.id, await passwords.hashPassword(password(req.body))); res.redirect(`/admin/teachers/${teacher.id}?success=password`); }
